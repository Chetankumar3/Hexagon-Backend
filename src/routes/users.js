...
router.get("/search", authenticateToken, async (req, res) => {
  try {
    ...
    const cacheKey = cacheKeys.users.search(q);
    ...
    const result = await cache.getOrSet(
      ...
      180 // Cache for 3 minutes (user searches are frequent but user data changes)
    );
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      await cache.delete(cacheKeys.users.byId(String(req.user._id)));
      await cache.delete(cacheKeys.users.byUsername(updatedUser.username));
      await cache.deletePattern('users:search:*'); // Invalidate all user searches
